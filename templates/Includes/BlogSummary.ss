<div class="blogSummary">
	...
	<p class="authorDate"><% _t('BlogSummary_ss.POSTEDBY', 'Posted by') %> <% if $AuthorMember %>$AuthorMember.Title<% else %>$Author.XML<% end_if %> <% _t('BlogSummary_ss.POSTEDON', 'on') %> $Date.Long | <a href="$Link#PageComments_holder" title="View Comments Posted">$Comments.Count <% _t('BlogEntry_ss.COMMENTS', 'Comments') %></a></p>
	<% if TagsCollection %>
		<p class="tags">
